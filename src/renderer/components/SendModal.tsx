...
import type { SendChain, FeeEstimate, SendResult } from '../types/wallet'

interface Props {
  chain: SendChain
  balance: string | null  // human-readable native balance
  symbol: string
  onClose: () => void
}
...
const CHAIN_LABEL: Record<SendChain, string> = {
  evm: 'EVM (Ethereum)',
  solana: 'Solana',
  cardano: 'Cardano'
}

const PLACEHOLDER_ADDR: Record<SendChain, string> = {
  evm: '0x...',
  solana: 'Base58 address...',
  cardano: 'addr1q...'
}

export function SendModal({ chain, balance, symbol, onClose }: Props) {
  ...
  const handleEstimateFee = async () => {
    ...
    setFee(null)
    try {
      const estimate = await window.wallet.estimateFee(chain, to.trim(), amount.trim())
      setFee(estimate)
    } catch (err) {
  ...
  const handleSend = async () => {
    ...
    try {
      let res: SendResult
      if (chain === 'evm')     res = await window.wallet.sendEvm(to.trim(), amount.trim())
      else if (chain === 'solana') res = await window.wallet.sendSolana(to.trim(), amount.trim())
      else                     res = await window.wallet.sendCardano(to.trim(), amount.trim())
      setResult(res)
      setStep('success')
    } catch (err) {
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button
            ...
            style={{
              ...
              color: 'var(--text-muted)', width: 28, height: 28, display: 'flex',
          ...
          <>
            ...
            <div>
              ...
              <input
                ...
                placeholder={PLACEHOLDER_ADDR[chain]}
                value={to}
        ...
        {step === 'error' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="btn btn-ghost" onClick={() => setStep('form')} style={{ flex: 1 }}>Try Again</button>
