...
export async function deleteWallet(): Promise<void> {
  ...
  await chrome.storage.local.remove(['wallet.enc', 'wallet.addresses', 'wallet.agw_signers'])
  await clearUnlockedMnemonic()
}
...
export async function loadSwapSessions(): Promise<unknown> {
  const r = await chrome.storage.local.get('wallet.swap_sessions')
  const m = r['wallet.swap_sessions']
  return (m && typeof m === 'object') ? m : {}
}

export function saveSwapSessions(map: unknown): void {
  chrome.storage.local.set({ 'wallet.swap_sessions': map }).catch(() => { /* see above */ })
}
...
// Mirrors secure-store.ts's agw-signers.enc. Abstract's portal exports the AGW's
// embedded signer private key, and holding it makes a watch-only AGW spendable —
// so this is the most sensitive thing in chrome.storage after wallet.enc, and it
// is encrypted under a key derived from the UNLOCKED mnemonic. Two consequences,
// both deliberate: a locked wallet cannot spend the AGW either, and replacing the
// wallet makes these blobs permanently unreadable (deleteWallet drops them).
//
...
async function loadAgwSigners(): Promise<Record<string, AgwSignerRecord>> {
  const r = await chrome.storage.local.get(AGW_SIGNERS_KEY)
  return (r[AGW_SIGNERS_KEY] ?? {}) as Record<string, AgwSignerRecord>
}
...
export async function clearAgwSigner(accountIndex: number): Promise<void> {
  ...
  await chrome.storage.local.set({ [AGW_SIGNERS_KEY]: map })
}
