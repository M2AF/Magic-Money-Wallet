...
- **dApp ready** — injects `window.ethereum` (EIP-1193 / EIP-6963), `window.solana` (Wallet Standard), `window.cardano.magicmoney` (CIP-30), and `window.midnight` (Midnight DApp Connector), plus the VESPR-authorized `window.cardano.vespr` and Lace-authorized `window.midnight.mnLace` compatibility keys and WalletConnect v2.
...
MagicMoney is a fully-fledged signer for EVM, Solana, and Cardano dApps, and connects to Midnight dApps.
...
- **Midnight (DApp Connector)** — registers on `window.midnight` for both API generations: under a stable UUID with `rdns` `info.chainlens.magicmoney` for 4.x dApps that enumerate the registry, and under the readable `magicmoney` key for legacy (≤3.x) dApps. With Lace's permission, a third entry is registered at `window.midnight.mnLace` carrying Lace's `io.lace.wallet` rdns — live Midnight dApps enumerate the registry but then filter it against a hardcoded rdns allowlist, so a wallet outside that list cannot be selected however correctly it registers. All three entries are the same MagicMoney-branded shim over the same approval-gated IPC routes, and the alias is skipped entirely when a genuine Lace is present. Implemented methods: `enable`/`connect`, `state`, `serviceUriConfig`/`getConfiguration`, `getUnshieldedAddress`, `getShieldedAddresses`, `getDustAddress`, `getUnshieldedBalances`, `getDustBalance`, `getConnectionStatus`, `signData`, `makeTransfer`, `submitTransaction`. `signData` produces a BIP-340 Schnorr signature from the unshielded (NIGHT) key — the same key behind the wallet's `mn_addr…` — and returns the spec's `{ data, signature, verifyingKey }`. Two signing modes, split on whether the payload is a readable message:

- **Readable text** signs the raw bytes, and `data` echoes the message. This is what the live ecosystem verifies against — Pulse Finance's backend rejects a prefixed signature outright — and it matches Lace, whose rdns this wallet answers to.
- **Binary payloads** sign behind the connector spec's `midnight_signed_message:<payload byte length>:` separator, with `data` carrying the exact prefixed bytes as hex.

That split is a security boundary, not a formatting one. Signing arbitrary bytes unprefixed with the transfer-authorising key would make `signData` an oracle a dApp could use to get a transaction signed from a "sign this message" prompt; requiring the separator for anything non-textual closes that, since a transaction segment is a serialization of hashes and keys and cannot masquerade as printable UTF-8. Payloads decode strictly from `text`, `hex`, or `base64` — lenient decoders silently truncate at the first invalid character, which would let a page display one message and have a shorter one signed. Approval-gated per call, and the prompt renders a decoding of the exact bytes being signed rather than a caption supplied alongside them.
- **Hardcoded dApp compatibility branding** — some dApps hardcode a wallet's label and icon instead of reading the provider metadata they were given. Where MagicMoney owns an authorized compatibility key, the injected provider narrowly replaces those wallet-row/dialog elements with the MagicMoney name and provider icon: the VESPR rows on `app.strikefinance.org` and `app.dexhunter.io`, and the Lace row on `pulsefinance.org`. It does not run on other sites, or when a genuine VESPR/Lace provider owns the key.
