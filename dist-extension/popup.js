...
function ExtApp() {
  ...
  reactExports.useEffect(() => {
    ...
    window.wallet._extConfirmBackup = async () => {
      const result = await original();
      setPage("setpassword");
      return result;
    };
    const originalImport = window.wallet.import.bind(window.wallet);
    window.wallet._extImport = async (m2) => {
      const result = await originalImport(m2);
      setPage("setpassword");
      return result;
    };
    return () => {
      delete window.wallet._extConfirmBackup;
      delete window.wallet._extImport;
    };
  }, []);
  if (page === "checking") {
